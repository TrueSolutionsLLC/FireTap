...
protocol HTTPTransport: Sendable {
    ...
    func sendReturningStatus(_ request: HTTPRequest, bearerToken: String?) async throws -> HTTPResponse
}
    ...
    func send(_ request: HTTPRequest, bearerToken: String?) async throws -> HTTPResponse {
        ...
        while true {
            ...
            do {
                ...
                case 200...299:
                    return HTTPResponse(data: data, status: http.statusCode, etag: etag, location: location)

                default:
                    if request.acceptableAdditionalStatuses.contains(http.statusCode) {
                        return HTTPResponse(data: data, status: http.statusCode, etag: etag, location: location)
                    }
                    switch http.statusCode {
                    case 401:
    ...
    func sendReturningStatus(_ request: HTTPRequest, bearerToken: String?) async throws -> HTTPResponse {
        ...
        while true {
            ...
            do {
                ...
                let location = http.value(forHTTPHeaderField: "Location")
                log.http(method: request.method.rawValue, url: request.resolvedURL, status: http.statusCode)
                let httpResponse = HTTPResponse(data: data, status: http.statusCode, etag: etag, location: location)
